...
class Abi(AbiBaseClass):
    ...
    def serialize_field(self, field: AbiStructField, value: Any) -> bytes:
        if field.type not in DEFAULT_TYPES:
            if t := self.find_type(field.type):  # handle custom types
                logging.debug(f"{field.type} uses internal type {t.type}")
            ...
            if s := self.find_struct(field.type):
                logging.debug(f"{field.type} uses internal struct {s.name}")
                if field.is_list:
                    return ListSerializable(
            ...
            if v := self.find_variant(field.type):
                logging.debug(f"{field.type} uses internal variant {v.name}")
                if field.is_list:
                    return ListSerializable(
                        [
